...
def determinePerfusionVolumes(caps,cap_center_coordinates,image,heart,threshold,vprint):
	numPts = heart.GetNumberOfPoints()
	heart_data = [0]*numPts
	heart_graph = Graph()
	...
	fastMarching(heart_graph,heart,cap_center_coordinates)
	heart = multipleSourceDistance(heart,heart_graph,100000000000,cap_heart_points)
	...
	for i in range(0,len(perfusion_data)):
		update_progress(i,len(perfusion_data),vprint)
...
def determineCenterlinePerfusionVolumes(caps,coordinates,weights,image,heart,threshold,out_filename,vprint):
	numPts = heart.GetNumberOfPoints()
	heart_data = [0]*numPts
	heart_graph = Graph()
	...
	heart = multipleSourceDistance(heart,heart_graph,100000000000,cap_heart_points,distances,weight_dict)
...
def createParser():
	...
	parser.add_argument('CAinflow', type=float, help='the amount of flow going to coronaries')
...
def main(args):
	...
	#centerlines = ['LCA_centerlines.vtp','RSA_centerlines.vtp','RCA_centerlines.vtp']
	...
	for i in centerlines:
		if not os.path.isfile('./'+'heart_' + i.split('.')[0] + '.vtu'):
			...
			centerline = intializeVTP(i,vprint)
			#centerline = getCenterline(centerline_main,i)
			coords = getCoords(centerline)
			weights = getWeights(centerline,option=0)
			volumes = determineCenterlinePerfusionVolumes(cap_names,coords,weights,image,heart,THRESHOLD,'heart_' + i.split('.')[0] + '.vtu',vprint)
		else:
	...
	for i in range(0,len(cap_names)):
		writeFlowFile('./FLOW-FILES/' + cap_names[i]+'.flow',(INFLOW*volumes[i]/tot_volume)*-1)
