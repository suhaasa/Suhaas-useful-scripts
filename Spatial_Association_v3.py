...
def addSpatialLag(model,factors):
	...
	for f in xrange(0,len(factors)):
		...
		for PtID in xrange(0,numPts):
			connectedPts_list = getConnectedVerticesWithinRadius(model,PtID,5)
			sum = 0
			count = 0
			for i in xrange(0,connectedPts_list.GetNumberOfIds()):
...
MF_connectivity = addSpatialLag(model,Mfactors)
HF_connectivity = addSpatialLag(model,Hfactors)
print('Start')
		...
		for MF in xrange(0,len(Mfactors)):
			for HF in xrange(0,len(Hfactors)):
				All_stats_surrogate[i_Sur][MF][HF] = determineSpatialAssociationStatistic( \
					MF_data_original[MF],HF_data_surrogate[HF],SL_MF_data_original[MF],SL_HF_data_surrogate[HF], \
					MF_means[MF],MF_stdev[MF],HF_means[HF],HF_stdev[HF])
				HF_SSS[i_Sur][HF] = calcSpatialSmoothingScalar(HF_data_surrogate[HF],SL_HF_data_surrogate[HF],HF_means[HF])
				MF_SSS[i_Sur][MF] = calcSpatialSmoothingScalar(MF_data_original[MF],SL_MF_data_original[MF],MF_means[MF])
